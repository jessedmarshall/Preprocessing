...
// updated: 2013.07.29
// changelog:
// now include in the log various answers
// updated: 2013.08.23 [11:44:14] changed so saves all images to the save folder
// refactor code to allow parallelization of several parts in later runs
// fixed issue relating to super short images
...
public class GCaMP_Register_Multiple_Days_Same_Target_3_1 implements PlugIn {
	...
	public void run(String arg) {
		...
		do {
			...
			String parent = new File((String) sourcedir.get(folderCounter)).toString();

			//--------------------------------------------------------
			//Establish destination directory
			// .getParentFile().toString();
			String destdirect = parent + File.separator;
			 // + "MotionCorrected" + File.separator;
			String destdirectNew = parent + File.separator;
			 // + "MotionCorrected-new" + File.separator;
			...
			String dfofdirect = parent + File.separator;
			 // + "DFOF" + File.separator;
			File dfofdir = new File(dfofdirect);
			String dfofdirectNew = parent + File.separator;
			 // + "DFOF-new" + File.separator;
			...
			// make the temporarty directory
			String tempdirect = parent + File.separator;
			 // + "Temp" + File.separator;
		...
		if (tSource == "Load") {
			...
			//Open the 42th slice of the selected file
			ImagePlus targFull = new Opener().openImage(tdir + tname, 42);
			if (targFull==null) {
				IJ.error("Error opening image.");
				return;
			}
		...
		for (int folderNo=0; folderNo<folderCounter; folderNo++) {
			...
			for (int z=0; z<flCurrent.length; z++) {
				...
				if (imgnorm.getNSlices()==1)
					new FileSaver(imgnorm).saveAsTiff(destF.get(folderNo)+title+"_turboreg.tif");
				else
					new FileSaver(imgnorm).saveAsTiffStack(destF.get(folderNo)+title+"_turboreg.tif");
				//--------------------------------------------------------

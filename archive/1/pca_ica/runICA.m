...
for i = 1:2:length(varargin)
    ...
    switch val
        ...
        case 'spatial_temporal_tradeoff'
            Mu = varargin{i+1};
        case 'ic_convergence_error'
            TermTolICs = varargin{i+1};
        case 'maxroundsics'
            MaxRoundsICs = varargin{i+1};
        case 'minnumpixels'
            minNumPixels = varargin{i+1};
        case 'ic_iterations'
            IC_iterations = varargin{i+1};
        case 'maxdist'
...
for repeat = 1:IC_iterations
    ...
    waitbarHandle = waitbar(0, 'iterating through ICA...');
    while (numiter<MaxRoundsICs) && ((1-minAbsCos)>TermTolICs)
        % reduce waitbar access
        if(mod(numiter,3)==0)
            waitbar(numiter/MaxRoundsICs,waitbarHandle)
        end

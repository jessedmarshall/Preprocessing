function [success] = downsampleHdf5Movie(inputFilePath, varargin)
	...
	if ~isempty(options.saveFolder)
		...
		options.newFilename = [options.saveFolder name '.h5'];
	end
	display(repmat('+',1,21))
	...
	try
		...
		for currentSubset=1:nSubsets
			...
			display(sprintf(['current location: ' num2str(currentSubsetLocation) '/' num2str(dataDim.z) '\noffset: ' num2str(offset) '\nblock: ' num2str(block)]));
			...
			downsampleMovieNested('downsampleDimension', 'space','downsampleFactor',options.downsampleFactor);
			% thisMovie = uint16(thisMovie);
			display(['subset class: ' class(inputMovie)])

			% save the movie
			if currentSubset==1
				createHdf5File(options.newFilename, options.outputDatasetName, inputMovie);
			else
					...
					case 'bilinear'
						...
						for frame=1:downY
						   ...
						   % inputMovie(:,frame,:) = downsampledFrame;
							if mod(frame,20)==0&nestedoptions.waitbarOn==1|frame==downY
					...
					case 'bilinear'
						...
						for frame=1:downZ
						   ...
						   % inputMovieDownsampled(1:downX,1:downY,frame) = downsampledFrame;
							if mod(frame,20)==0&nestedoptions.waitbarOn==1|frame==downZ

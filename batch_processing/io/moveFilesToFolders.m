function [success destFolders] = moveFilesToFolders(pathToSrc,pathToDest,varargin)
    ...
    options.srcFolderFilterRegexp = '2014';
    % this regexp is used to search the destination directory
    options.srcSubfolderFileFilterRegexp = 'recording.*.txt';%xml
    %
    options.srcSubfolderFileFilterRegexpExt = '.txt';
    % get options
        ...
        for i = 1:nFolders
            ...
            if ~isempty(loopFolderFiles)
                ...
                for j = 1:length(loopFolderFiles)
                    ...
                    iFile = strrep(loopFolderFiles{j},options.srcSubfolderFileFilterRegexpExt,'');

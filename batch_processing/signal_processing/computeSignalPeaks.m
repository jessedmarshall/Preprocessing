function [signalPeaks, signalPeaksArray] = computeSignalPeaks(signalMatrix, varargin)
    % controller to digitize [0,1] input analog signals based on peaks in the signal.
    % biafra ahanonu
    % started: 2013.10.28
    % inputs
...
function [testpeaks] = computePeakForSignal(inputSignal, varargin)
    ...
    % remove obvious non-peaks
    switch options.detectMethod
        case 'diff'
            % get the differential
            testpeaks = testpeaks(rawInputSignal(testpeaks)>1.96*rawInputSignalStd);
            options.nFramesShift = 0;
        case 'raw'
            %
        otherwise

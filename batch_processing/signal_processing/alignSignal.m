function [alignedSignal] = alignSignal(responseSignal, alignmentSignal,timeSeq,varargin)
	...
	try
		...
		% get the indices as a [nSignals (nAlignPoints*nAlignmemts)]
		...
		switch options.returnFormat
			case 'perStimSignalResponse'
				...
				A = reshape(alignedSignal', [nAlignmemts nAlignPoints nSignals]);
				C = permute(A,[1 3 2]);
				C = reshape(C,[],size(A,2),1);
				alignedSignal = C;
			case 'count:[nSignals nAlignmemts]'
				% [nSignals nAlignPoints]
				alignedSignal = reshape(alignedSignal', [nAlignPoints nAlignmemts nSignals]);
			...
			case 'perSignalStimResponseCount'
				% [nSignals nAlignPoints*nAlignmemts]
				% alignedSignal = alignedSignal;
				% alignedSignal = nansum(reshape(alignedSignal', [nAlignPoints nAlignmemts nSignals]),2);
			...
			case 'perSignalStimResponseMean'
				...
				alignedSignal = nanmean(reshape(alignedSignal', [nAlignPoints nAlignmemts nSignals]),2);
				% squeeze to 2D, flip so have [nSignals nAlignPoints] matrix
				alignedSignal = squeeze(alignedSignal);
				if nAlignPoints~=1
					alignedSignal = alignedSignal';

function [exitSignal ostruct] = playMovie(inputMovie, varargin)
	...
	redWhiteMap = [R', G', B'];
	options.colormapColor = redWhiteMap;
	options.colormapColorList = [redWhiteMap,options.colormapColorList];
	% ========================
	...
	fig1 = figure(42);close(fig1);fig1 = figure(42);
	if isempty(options.extraTitleText)
		options.extraTitleText = '';
	else
		options.extraTitleText = [10,options.extraTitleText];
	end
	suptitleHandle = suptitle([10,10,10,'e:exit    q:exit all    g:goto frame    p:pause/play    r:rewind    f:forward    j:contrast    l:label    i:imagej',10,'c:crop    d:\DeltaF/F    a:downsample    n:normalize    m:\Deltacolormap    +:speed    -:slow    ]:+1    [:-1    dims: ' num2str(size(inputMovie)),'    ',strrep(options.extraTitleText,'\','/'),10,repmat(' ',1,7),10,10]);
			 ...
			 %               'alimmode','manual');
			    ...
			    if ~isempty(options.primaryPoint)
			    	if length(options.primaryPoint(:,1))==1
			    		plot(options.primaryPoint(:,1), options.primaryPoint(:,2), 'k+', 'Markersize', 42)
			    	else
			...
			switch double(keyIn)
				...
				case 106 %'j' %change contrast
					...
					elseif usrIdxChoice==1
						...
						answer = inputdlg({'max','min'},'cell extraction estimates',1,{num2str(maxMovie(usrIdxChoice)),num2str(minMovie(usrIdxChoice))})

...
biafra ahanonu
updated starting: 2013.08.30
updated: 2013.08.30 [10:17:20] notifies which step it is currently at
...
updated: 2013.09.12 [19:23:23] script altered so it now should be called via run("mm processMovies 1",getArgument);
updated: 2013.09.19 [10:56:57] script now exits with different system error codes depending on
updated: 2013.10.23 [20:36:29] removed band-pass filtering from pre-processing
TODO: refactor code (e.g. the original script has a hatred of functions) to allow parallelization of several parts in later runs

ERROR CODES for batch processing
0 - normal exit, everything is fine
1 - do not have /target/target.tif in the parent folder
2 - have too many DFOF/MotionCorrected folders
3 - Missing a *.tif in the directory chosen
4 - WTF happened?
...
public class mm_processMovies_1 implements PlugIn {
	...
	private void mainFxn(String argj){
		...
		// whether to save turboreg coordinates
		String scyn = "";
		// the path to the target file, for turboreg
		String targetpath = "";
		// use to determine feature selection (dark or light)
		String dkslts = "";
		// height and width of target file
		int width;
		int height;
		// roi of target file, for turboreg
		Roi roi;
		//
		String tLoadDir = "";
		...
		// }
		// else {
		// 	// IJ.showMessage("Please clean up your old motion correction folders!");
		// 	System.exit(2);
		// 	// return;
		// }
		destF.add(destdirect);
		...
		// check that the target.tif exists, if not, exit the plugin
		File targetFilePointer = new File(targetFile);
		if(targetFilePointer.exists() && targetFilePointer.isFile() && targetFilePointer.canRead()) {
		...
		for (int folderNo=0; folderNo<folderCounter; folderNo++) {
			...
			for (int z=0; z<flCurrent.length; z++) {
				...
				// printTime(startTime);
				// IJ.log("low-pass filtering...");
				// ImagePlus imgnorm = new Duplicator().run(imFresh);
				// imFresh.close();
				// imFresh.flush();
				//--------------------------------------------------------
			...
			for (int w=0; w<flCurrent.length; w++){
				String x = flCurrent[w].substring(1, 7);
				int n = 0;
				String toconcat = "";
			...
			// run("Save HDF5 File", dfofF.get(folderNo)+firstName+"_fullSession_DFOF.hdf5 1");
			full.close();
			full.flush();
		...
		// IJ.log("\\Clear");
		// IJ.log(dfofF.get(folderNo)+firstName+"_fullSession_DFOF"+".tif");
		// IJ.saveAs("Text", concatBaseNamePath + ".log.txt");
		printTime(startTime);
	}
